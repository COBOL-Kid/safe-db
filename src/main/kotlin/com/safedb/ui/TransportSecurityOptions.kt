...
private val VERIFY_IDENTITY = TransportSecurityOption(
...
private val VERIFY_CA = TransportSecurityOption(
...
private val ENCRYPT_ONLY = TransportSecurityOption(
...
private val DISABLED = TransportSecurityOption(
    TransportSecurityMode.Disabled,
    "No encryption",
    "Sends database traffic without TLS. Use only for a trusted local connection.",
)
private val ORACLE_VERIFY_IDENTITY = VERIFY_IDENTITY.copy(
    label = "TCPS: verify certificate and server identity",
    description = "Uses the Oracle wallet to validate trust and checks the database server identity.",
)
private val ORACLE_VERIFY_CA = VERIFY_CA.copy(
    label = "TCPS: verify certificate only",
    description = "Uses the Oracle wallet to validate trust without checking the database server identity.",
)

internal fun transportOptionsFor(dialect: Dialect): List<TransportSecurityOption> = when (dialect) {
    Dialect.Postgres, Dialect.MySql -> listOf(VERIFY_IDENTITY, VERIFY_CA, ENCRYPT_ONLY, DISABLED)
...
internal fun displayedTransportMode(dialect: Dialect, mode: TransportSecurityMode): TransportSecurityMode =
    when {
        dialect == Dialect.Mssql && mode == TransportSecurityMode.VerifyCa -> TransportSecurityMode.VerifyIdentity
        dialect == Dialect.Oracle && mode == TransportSecurityMode.EncryptOnly -> TransportSecurityMode.VerifyCa
        else -> mode
    }

internal fun normalizedTransportMode(dialect: Dialect, mode: TransportSecurityMode): TransportSecurityMode =
    displayedTransportMode(dialect, mode)
...
internal fun transportCompatibilityWarning(dialect: Dialect, mode: TransportSecurityMode): String? = when {
