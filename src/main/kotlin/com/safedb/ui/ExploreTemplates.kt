...
internal object BuiltinExploreTemplateCatalog : ExploreTemplateCatalog {
    override fun builtinTemplates(): List<ExploreTemplateDefinition> = listOf(
...
internal fun listExploreTemplates(
    ...
    val builtin = catalog.builtinTemplates().map { template ->
    ...
    val user = catalog.userTemplates().map { template ->
        val result = template.build(sample, fields)
        ExploreTemplateListItem(
            id = template.id,
            name = template.name,
...
private fun buildBreakdownTemplate(
    ...
    val dimension = firstDimensionField(fields)
        ?: return ExploreTemplateBuildResult.Unavailable("Needs at least one field in the sample.")
...
private fun firstNumericField(fields: List<ExploreFieldOption>): ExploreFieldOption? =
    fields.firstOrNull { it.category in NUMERIC_CATEGORIES && !looksLikeUniqueIdentifier(it.column, it.label) }

private fun buildTrendOverTimeTemplate(
    sample: QueryResult,
    fields: List<ExploreFieldOption>,
): ExploreTemplateBuildResult {
    val temporal = fields.firstOrNull { it.category in TEMPORAL_CATEGORIES }
        ?: return ExploreTemplateBuildResult.Unavailable("Needs a date or datetime column in your sample.")
    val numeric = firstNumericField(fields)
        ?: return ExploreTemplateBuildResult.Unavailable("Needs a numeric column in your sample.")
    return ExploreTemplateBuildResult.Ready(
        ExploreConfig(
            rowDimensions = listOf(
                temporal.asDimension().copy(
...
private fun buildCompareCategoriesTemplate(
    ...
    if (dimensions.size < 2) {
        return ExploreTemplateBuildResult.Unavailable("Needs at least two groupable fields in your sample.")
    }
...
private fun dimensionFields(fields: List<ExploreFieldOption>): List<ExploreFieldOption> =
    fields.filterNot { looksLikeUniqueIdentifier(it.column, it.label) }
...
private fun looksLikeUniqueIdentifier(column: String, label: String): Boolean {
    ...
    return normalized == "id" || normalized.endsWith("_id") || normalized.endsWith(" id") ||
        column.lowercase().endsWith("_id") || column.lowercase() == "id"
}
