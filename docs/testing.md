...
Use the wrapper, never system Gradle. Do not run `run` alongside daemon-less builds. `check` requires at least 306 desktop and 526 shared tests, with 90% and 86% line-coverage floors respectively. Inspect JUnit XML if discovery is uncertain.
...
A maintainer applies the `ci:run` label to a pull request to run `check` and required static-MySQL integration. Remove and reapply the label after new commits to rerun. Workflow-only pull requests run workflow lint when labeled.
