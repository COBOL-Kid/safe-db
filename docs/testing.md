...
Use the wrapper, never system Gradle. Do not run `run` alongside daemon-less builds. `check` requires at least 309 desktop, 540 shared, and 44 mcp tests, with 90% and 85% line-coverage floors for desktop and shared respectively. Inspect JUnit XML if discovery is uncertain.
