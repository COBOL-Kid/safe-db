...
Use the wrapper, never system Gradle. Do not run `run` alongside daemon-less builds. `check` requires at least 305 desktop and 526 shared tests, with 91% and 86% line-coverage floors respectively. Inspect JUnit XML if discovery is uncertain.
