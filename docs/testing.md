...
Windows Conveyor packaging is a separate manual **Run workflow** (`.github/workflows/conveyor.yml`). It builds a signed Windows site on Linux with Azure Trusted Signing, uploads `output/` as a 14-day artifact, and publishes to `gs://safedb-download` only from `main` when the `publish` input is true. See [packaging.md](packaging.md). Environment `conveyor` plus Azure OIDC, the existing Conveyor root key, and GCP WIF are configured in the GitHub UI. Do not commit signing keys or cloud tokens.
