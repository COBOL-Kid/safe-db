...
class ExploreRecipesTest {
    ...
    fun ambiguousAndIncompatibleFieldsStayUnresolvedUntilManualMapping() {
        val ambiguousSample =
            QueryResult(
                columns =
                    listOf(
                        ResultColumn("t0__amount", "decimal"),
                        ResultColumn("t1__amount", "decimal"),
                    ),
                rows = emptyList(),
                rowCount = 0,
                truncated = false,
        ...
        assertEquals(
            ...
            resolveRecipeFields(
                    ...
                    ambiguousSample,
                    spec(),
                    mapOf("old__amount" to "t1__amount"),
                )
                .resolved["old__amount"],
        )
        assertTrue(!compatibleTypes("decimal", "varchar"))
        val incompatibleSample =
            QueryResult(
    ...
    fun remapRewritesPivotWorksheetAndFormulaReferences() {
                        ...
                        calculations =
                            listOf(
                                ...
                                WorksheetCalculation.Window(
                                    ...
                                    WorksheetValueRef.Column("old"),
                                    groupColumn = "old_group",
                                    restartColumns = listOf("old_restart"),
                                ),
                            ),
                    ),
            )
        ...
        val mapped =
            remapRecipe(
                recipe,
                mapOf("old" to "new", "old_group" to "new_group", "old_restart" to "new_restart"),
            )
        ...
        assertEquals(
            WorksheetValueRef.Calculation("calc"),
            mapped.worksheet?.columnLayout?.last()?.ref,
        )
        ...
        assertEquals(
            "[sum] / [count]",
            (calculations[1] as WorksheetCalculation.GroupFormula).formula,
        )
        assertEquals(
            "new_group",
            (calculations[1] as WorksheetCalculation.GroupFormula).groupColumn,
        )
        assertEquals("new_group", (calculations[2] as WorksheetCalculation.Aggregate).groupColumn)
        assertEquals(
            "new_restart",
            (calculations[3] as WorksheetCalculation.Window).restartColumns.single(),
        )
    }
    ...
    private fun recipe(fields: List<RecipeField>) =
        ExploreRecipe(
            id = "r",
            name = "Recipe",
            createdAt = "1",
