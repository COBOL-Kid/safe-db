...
class PlanParsersTest {
    ...
    fun postgresJsonNormalizesRelationsJoinsAndBlockingOperations() {
        ...
        assertNotNull(plan)
        assertEquals(PlanAccessMethod.BoundedLookup, plan.relations.first { it.alias == "t0" }.method)
    ...
    fun oraclePlanTableRowsNormalizeIndexJoinAndSortEvidence() {
        val plan = normalizeOraclePlan(
            listOf(
                ...
                OraclePlanRow(3, 2, "TABLE ACCESS", "BY INDEX ROWID", "APP", "ORDERS", "t0", 10, 4.0),
    ...
    fun malformedAndUnknownShapesRemainUnavailableToCallers() {
        ...
        assertNull(parseSqlServerPlan("<!DOCTYPE x [<!ENTITY ext SYSTEM 'file:///etc/passwd'>]><x>&ext;</x>"))
    }
}
