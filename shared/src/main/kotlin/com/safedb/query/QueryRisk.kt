...
private enum class PredicateConstant {
    True,
    False,
    Unknown,
    Empty,
...
private fun nodeId(node: FilterNode): String =
    when (node) {
...
private enum class AccessPathState {
    Compatible,
    Incompatible,
    Unknown,
}
...
private fun isScanProneText(filter: com.safedb.model.FilterSpec): Boolean =
    when (filter.op) {
        FilterOp.Contains,
...
private fun joinedColumns(alias: String, joins: List<com.safedb.model.JoinSpec>): Set<String> =
...
private fun TableInfo.subject(alias: String, column: String? = null) =
    RiskSubject(tableAlias = alias, schema = schema, table = name, column = column)

private fun signalMessage(signal: RiskSignal): String =
    when (signal.code) {
        RiskSignalCode.NoEffectiveRestriction ->
