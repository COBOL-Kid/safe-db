...
internal fun normalizeTableSize(rowEstimate: Double?, confidence: EvidenceConfidence): TableSizeEstimate = when {
    rowEstimate == null || !rowEstimate.isFinite() || rowEstimate < 0.0 -> TableSizeEstimate(
        coverage = MetadataCoverage.unavailable("row_estimate_unavailable"),
    )
    rowEstimate < 10_000.0 -> TableSizeEstimate(
        TableSizeClass.Small,
        MetadataCoverage.complete(),
        confidence,
    )
    rowEstimate < 1_000_000.0 -> TableSizeEstimate(
        TableSizeClass.Medium,
        MetadataCoverage.complete(),
        confidence,
    )
    else -> TableSizeEstimate(
        ...
        tables.map { table ->
            val tableIndexes = indexesByTable[table].orEmpty()
                ...
                .map { (name, rows) ->
                    ...
                    IndexInfo(
                        name = name,
                        columns = rows.filterNot(MetadataIndex::included).mapNotNull(MetadataIndex::column),
                        includedColumns = rows.filter(MetadataIndex::included).mapNotNull(MetadataIndex::column),
                        kind = first.kind,
                        supportsEquality = first.supportsEquality,
                        isUnique = first.unique,
                        isPrimary = first.primary,
                        keys = rows.filterNot(MetadataIndex::included).map { row ->
