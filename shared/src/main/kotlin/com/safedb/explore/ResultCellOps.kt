...
internal fun resultCellText(cell: ResultCell?): String = when (cell) {
    null, ResultCell.Null -> ""
...
internal fun resultCellDecimal(cell: ResultCell?): BigDecimal? = when (cell) {
    is ResultCell.IntegerCell -> BigDecimal.valueOf(cell.value)
    is ResultCell.FloatCell -> BigDecimal.valueOf(cell.value)
    is ResultCell.TextCell -> cell.value.text.trim().takeIf(String::isNotEmpty)?.toBigDecimalOrNull()
    else -> null
}
