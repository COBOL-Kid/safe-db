...
fun buildJdbcUrl(def: ConnectionDef, password: String): String = when (def.dialect) {
...
internal fun createDataSourceConfig(def: ConnectionDef, password: String): HikariConfig {
    ...
    when (def.dialect) {
        Dialect.Postgres -> applyPostgresSsl(config, def)
        Dialect.MySql -> applyMySqlSsl(config, def)
...
private fun applyPostgresSsl(config: HikariConfig, def: ConnectionDef) {
