...
private fun ResultCell.decimalOrNull(): BigDecimal? = when (this) {
    is ResultCell.IntegerCell -> value.toBigDecimal()
    is ResultCell.FloatCell -> value.toBigDecimal()
    is ResultCell.TextCell -> value.text.toBigDecimalOrNull()
    else -> null
}

private fun ResultCell.text(): String = when (this) {
    ResultCell.Null -> ""
    is ResultCell.BoolCell -> value.toString()
    is ResultCell.IntegerCell -> value.toString()
    is ResultCell.FloatCell -> value.toString()
    is ResultCell.TextCell -> value.text
    is ResultCell.BinaryCell -> value.base64
}
