...
object IntegrationAssumptions {
    ...
    val postgresHost: String get() = env["SAFEDB_TEST_POSTGRES_HOST"] ?: "localhost"
    val postgresPort: Int get() = env["SAFEDB_TEST_POSTGRES_PORT"]?.toIntOrNull() ?: 5432
    val postgresUser: String get() = env["SAFEDB_TEST_POSTGRES_USER"] ?: "postgres"
    val postgresPassword: String get() = env["SAFEDB_TEST_POSTGRES_PASSWORD"] ?: "postgres"
    val postgresDatabase: String get() = env["SAFEDB_TEST_POSTGRES_DATABASE"] ?: "safedb_test"

    fun assumeMysqlAvailable() {
        requireOrAssume(
            available = isMysqlReachable(),
            required = env["SAFEDB_TEST_REQUIRE_MYSQL"].equals("true", ignoreCase = true),
            message = "Seeded MySQL fixture not reachable at $mysqlHost:$mysqlPort; " +
    ...
    fun assumePostgresAvailable() {
        requireOrAssume(
            ...
            message = "Seeded PostgreSQL fixture not reachable at $postgresHost:$postgresPort; " +
