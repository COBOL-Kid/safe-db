...
class AdapterTest {
    ...
    fun jdbcUrlsCoverAllSupportedDialectsAndTransportModes() {
        assertEquals(
            ...
            buildJdbcUrl(connection(Dialect.Postgres, 5432, TransportSecurityMode.VerifyIdentity), "pw"),
        )
        assertEquals(
            "jdbc:sqlserver://db.example.com:1433;databaseName=app;applicationName=safe-db",
            buildJdbcUrl(connection(Dialect.Mssql, 1433, TransportSecurityMode.EncryptOnly), "pw"),
        )
        assertEquals(
            "jdbc:oracle:thin:@//db.example.com:1521/app",
            buildJdbcUrl(connection(Dialect.Oracle, 1521, TransportSecurityMode.Disabled), "pw"),
        )
        assertEquals(
