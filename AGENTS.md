...
- **Safety** — read-only selects, row limit (max 1 000, default 100), 10 s query timeout, blocked schemas, filter literal type validation, and an EXPLAIN cost guard that requires confirmation when cost is unavailable or above threshold
