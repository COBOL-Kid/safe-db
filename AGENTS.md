...
- `scripts/seed_mysql.sh` — Gradle-backed Compose/Kotlin MySQL seeder; streams generated SQL into the local MySQL test DB for a larger reporting fixture (~50k orders by default; pass args directly, e.g. `--orders 20000 --customers 5000`); leaves `connections.json` and `query_history.json` alone. `scripts/seed_mysql.sh --static` loads the smaller bundled `testdata_mysql.sql` fixture. `scripts/seed_mysql.sh --reset-state` additionally wipes connections + history (saved queries and settings are always left untouched). `scripts/seed_mysql.sh --reset` drops the DB before generated seeding — destructive, use deliberately. Auto-connects to `localhost:3306` as `root` into `safedb_test`, overridable via `SAFEDB_TEST_MYSQL_*` env vars. If no `mysql` client is on PATH, auto-detects a running mysql/mariadb Docker container and runs the client inside it via `docker exec` (pin one with `SAFEDB_TEST_MYSQL_DOCKER=<name>`). When password is unset, reads `MYSQL_ROOT_PASSWORD` from the container env (docker-exec and host-client modes against localhost). Primary Gradle form: `cd compose-app && ./gradlew seedMysql -PseedMysqlArgs="--orders 20000"`.
...
- **Smoke gate (optional, needs DB):** `pnpm test:smoke` after seeding/configuring databases:
  ...
  - `mysql_smoke` — seed with `scripts/seed_mysql.sh`, then set `SAFEDB_TEST_MYSQL_HOST`, `SAFEDB_TEST_MYSQL_DATABASE`, `SAFEDB_TEST_MYSQL_USER` (optional `SAFEDB_TEST_MYSQL_PASSWORD`, `SAFEDB_TEST_MYSQL_PORT`; empty password is valid for local root).
