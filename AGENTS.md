...
- `check` currently enforces minimum unit discovery of 191 desktop and 341 shared test cases, with no JUnit XML failures or errors. Raise a floor when deliberately adding tests; do not lower it to hide a discovery regression.
...
- The preview renderer currently writes 36 PNGs: 16 main-app images at 1280×832, 2 narrow connection-form images at 840×900, and 18 Explore/recipe images at 1120×760, in light and dark modes.
...
- Launch-profile JSON is the only custom trust-store configuration path for verified PostgreSQL, MySQL, and SQL Server connections. Without a launch profile, MySQL and SQL Server use normal JVM trust; PostgreSQL must retain pgjdbc's standard trust and client-certificate behavior. Saved connections cannot override trust configuration. Oracle verification remains wallet-based.
